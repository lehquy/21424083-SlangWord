...
public class QuestionFrame extends JFrame implements ActionListener {
	...
	QuestionFrame(int type) {
		...
		// A Label
		JLabel label = new JLabel("Tim cau tra loi dung");
	...
	public void answer(int ans) {
		if (s[ans].equals(s[5])) {
			...
			JOptionPane.showMessageDialog(this, "Dap an dung");

		} else {
			JOptionPane.showMessageDialog(this, "Dap an sai", "Inane error", JOptionPane.ERROR_MESSAGE);
			if (ans == 1)
